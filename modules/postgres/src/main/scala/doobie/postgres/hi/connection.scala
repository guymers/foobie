...
object connection {
   ...
   * channel is NOT sanitized; it cannot be passed as a parameter and is simply
   ...
   * Note that neither the channel nor the payload are sanitized; neither can be
   * passed as parameters and are simply interpolated into the statement. DO NOT
  ...
  /**
   * Construct a program that starts listening on the given channel. Note that
   * the channel is NOT sanitized; it cannot be passed as a parameter and is
   * simply interpolated into the statement. DO NOT pass user input here.
   */
  def pgListen(channel: String): ConnectionIO[Unit] =
    execVoid("LISTEN " + channel)

  /**
   * Construct a program that stops listening on the given channel. Note that
   * the channel is NOT sanitized; it cannot be passed as a parameter and is
   * simply interpolated into the statement. DO NOT pass user input here.
   */
