...
class StubConnection(valid: Int => Boolean = _ => true) extends Connection {
  ...
  private val _preparedStatements = mutable.ListBuffer.empty[StubPreparedStatement]
  def preparedStatements = _preparedStatements.toList
  ...
  override def prepareStatement(sql: String): PreparedStatement = newPreparedStatement(sql)
  ...
  private def newPreparedStatement(sql: String) = {
    val fail = sql == StubConnection.InvalidSQL
    val results = {
      if (sql.trim == StubConnection.Select1) {
        List(Vector[AnyRef](1: java.lang.Integer))
      } else Nil
    }
    val ps = new StubPreparedStatement(results, fail)
    val _ = _preparedStatements.addOne(ps)
    ps
  }
