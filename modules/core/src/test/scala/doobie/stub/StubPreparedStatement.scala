...
class StubPreparedStatement(results: List[Vector[AnyRef]] = Nil, fail: Boolean = false) extends PreparedStatement {
  private var closed = false

  private val _resultSets = mutable.ListBuffer.empty[ResultSet]
  def resultSets = _resultSets.toList
  ...
  override def executeQuery(): ResultSet = newResultSet()
  override def executeQuery(sql: String): ResultSet = ???

  private def newResultSet() = {
    if (fail) throw new SQLSyntaxErrorException()
    val rs = new StubResultSet(results)
    val _ = _resultSets.addOne(rs)
    rs
  }
