...
sealed abstract class Get[A](
  ...
  /**
   * Apply `f` to values retrieved by this `Get`. Prefer `tmap` when possible
   * because it will allow for better diagnostics when checking queries. Note
   * that `null` values will not be transformed, so you do not need to (nor can
  ...
  /**
   * Equivalent to `map`, but accumulates the destination type in the type stack
   * for improved diagnostics. Prefer this method when you have concrete types
   * or an available TypeName.
   */
