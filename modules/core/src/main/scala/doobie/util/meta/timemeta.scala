...
trait TimeMeta {
  ...
  @deprecated(
    "Use doobie.implicits.javatimedrivernative instead. If you are using a database which doobie directly integrates with, " +
  ...
  /**
   * Use this import if you want to use the driver-native support for
   * java.time.* types. This means that the java.time value is passed straight
   * to the JDBC driver you're using without being converted to java.sql.* types
   * (Unlike doobie.implicits.legacy.instant/localdate)
   */
...
trait TimeMetaInstances { this: MetaConstructors =>
  ...
  implicit val JavaTimeInstantMeta: Meta[java.time.Instant] =
    Basic.one[java.time.Instant](
      ...
      _.getObject(_, classOf[java.time.Instant]),
      _.setObject(_, _),
      _.updateObject(_, _),
    )
  ...
  implicit val JavaZonedDateTimeMeta: Meta[java.time.ZonedDateTime] =
    Basic.one[java.time.ZonedDateTime](
      TimestampWithTimezone,
      List(Char, VarChar, LongVarChar, Date, Time, Timestamp),
      _.getObject(_, classOf[java.time.ZonedDateTime]),
      _.setObject(_, _),
      _.updateObject(_, _),
    )
